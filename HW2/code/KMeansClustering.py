...
ClusterResult = namedtuple('ClusterResult', ['clusters', 'clusterIndex'])

@logArgs(logger)
def KMeansCluster(pts, K):
    '''
    Args:
    ...
    Returns:
        ClusterResult
    '''
    return ClusterResult([Cluster(0, pts[0], pts)], lambda x: 0)
...
if __name__ == '__main__':
    ...
    clusterResult = KMeansCluster(pts, K = NClusters)
    for cluster in clusterResult.clusters:
        print('Cluster {0} - centroid {1}'.format(cluster.clusterNumber, cluster.centroid))
